...
ddef query_db(query, args=(), one=False, commit=False, fetch=True):
...
def register():
    ...
    if request.method == "POST":
        ...
        # Save QR to the correct folder
        qr_path = os.path.join(QR_FOLDER, f"{plate_number}.png")
        img.save(qr_path)

        # Render-safe for Render or Local
        if "RENDER" in os.environ:
            import base64
            with open(qr_path, "rb") as f:
                qr_base64 = base64.b64encode(f.read()).decode()
            # In template, use <img src="data:image/png;base64,{{ qr_base64 }}">
            return render_template("success.html", plate_number=plate_number, qr_base64=qr_base64)
        else:
            # Local: serve via static folder
...
def dashboard_only():
    ...
    total_registered_today = query_db("SELECT COUNT(*) AS c FROM registered WHERE DATE(registration_date)=CURRENT_DATE()", one=True)["c"]

    total_users = query_db("SELECT COUNT(*) AS c FROM registered", one=True)["c"]

    active_parked = query_db("SELECT COUNT(*) AS c FROM parking_logs WHERE time_out IS NULL", one=True)["c"]

    entries_today = query_db("SELECT COUNT(*) AS c FROM parking_logs WHERE DATE(time_in)=CURRENT_DATE()", one=True)["c"]

    exits_today = query_db("SELECT COUNT(*) AS c FROM parking_logs WHERE DATE(time_out)=CURRENT_DATE()", one=True)["c"]

    overstay_count = query_db(
        "SELECT COUNT(*) AS c FROM parking_logs WHERE time_out IS NULL AND DATE(time_in) < CURRENT_DATE()",
        one=True
    )["c"]
...
def view_overstay():
    ...
    return render_template(
        "admin_dashboard.html",
        section="overstay",          # ✅ REQUIRED
        vehicles=vehicles,          # ✅ REQUIRED
        total_registered_today=users_today,
        total_registered=total_registered,
        active_parked=active_parked,
        entries_today=entries_today,
